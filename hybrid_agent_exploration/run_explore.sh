#!/bin/bash
# ============================================================================
# PSC ML Random Pipeline Explorer — 使用 OpenCode + Kimi K2.6 进行随机管线探索
#
# 用法:
#   bash run_explore.sh           # 默认跑 10 轮
#   bash run_explore.sh 50        # 跑 50 轮
#   bash run_explore.sh 100       # 跑 100 轮
#
# 如果中断后想续跑:
#   bash run_explore.sh --resume 100
# ============================================================================
...
opencode run ${RESUME_FLAG} \
    --model "${MODEL}" \
    --agent "${AGENT}" \
    --dangerously-skip-permissions \
    --title "PSC Exploration: ${N_RUNS} runs" \
    --dir "${PROJECT_DIR}" \
    "你是一个钙钛矿太阳能电池（PSC）机器学习管线探索Agent。

## 任务

执行 ${N_RUNS} 轮加权随机管线探索。每一轮随机组合5层方法（数据源→特征→模型→评估→筛选），执行完整的ML管线，记录结果。

$(if [ -n "${RESUME_FLAG}" ]; then echo "## 断点续跑模式"; echo "先检查 results/checkpoint.json，从上次中断的位置继续。"; fi)
