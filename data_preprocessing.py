...
This script loads chemical and JV data, calculates Delta_PCE, and performs data cleaning.
"""
...
def main():
    ...
    # Step 2: Calculate Delta_PCE
    print("\n[Step 2] Calculating Delta_PCE...")
    df['Delta_PCE'] = df['jv_reverse_scan_pce'] - df['jv_reverse_scan_pce_without_modulator']
    print(f"Delta_PCE calculated. Range: [{df['Delta_PCE'].min():.4f}, {df['Delta_PCE'].max():.4f}]")

    # Step 3: Select relevant columns for QSPR analysis
    print("\n[Step 3] Selecting relevant columns for QSPR analysis...")
    ...
    selected_cols = []
    for col in identifier_cols + chemical_features + target:
    ...
    # Step 4: Missing value analysis before cleaning
    ...
    # For QSPR analysis, we need complete cases for features and target
    analysis_cols = chemical_features + target
    missing_in_analysis = df_selected[analysis_cols].isnull().sum(axis=1)

    # Keep rows with complete data for analysis
    ...
    print(f"Retained {final_samples} complete samples")
    ...
    # Additional statistics for Delta_PCE
    ...
    with open(OUTPUT_SUMMARY, 'w') as f:
        ...
        for col in df_selected.columns:
            ...
            f.write(f"{col:30s}: {missing_count:6d} ({missing_pct:6.2f}%)\n")
        f.write("\n")
