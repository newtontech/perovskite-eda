...
print(f"   Molecules with <4 atoms (TT=0): {np.sum(tt_fps.sum(axis=1) == 0)}")
print()
...
ap_freq_vals = ap_bit_freq[ap_bit_freq > 0]
if len(ap_freq_vals) > 0:
    ax1.hist(ap_freq_vals, bins=50, color='#3498db', alpha=0.7, edgecolor='black')
ax1.set_xlabel('Bit Frequency', fontsize=11)
ax1.set_ylabel('Count', fontsize=11)
ax1.set_title(f'AP Bit Frequency Distribution ({len(ap_freq_vals)} active bits)', fontsize=12, fontweight='bold')

ax2 = plt.subplot(2, 3, 2)
tt_freq_vals = tt_bit_freq[tt_bit_freq > 0]
if len(tt_freq_vals) > 0:
    ax2.hist(tt_freq_vals, bins=50, color='#e74c3c', alpha=0.7, edgecolor='black')
ax2.set_xlabel('Bit Frequency', fontsize=11)
ax2.set_ylabel('Count', fontsize=11)
ax2.set_title(f'TT Bit Frequency Distribution ({len(tt_freq_vals)} active bits)', fontsize=12, fontweight='bold')
...
# Only do PCA if we have active bits
if np.sum(ap_active) > 0 and np.sum(tt_active) > 0:
    ...
    print("   Warning: Not enough active bits for PCA")
    ap_pca = None
    tt_pca = None
...
ap_tt_combined = np.hstack([ap_fps_filtered, tt_fps_filtered]) if ap_fps_filtered.shape[1] > 0 and tt_fps_filtered.shape[1] > 0 else ap_fps_filtered if ap_fps_filtered.shape[1] > 0 else tt_fps_filtered
...
if len(ap_features) > 0 and len(tt_features) > 0:
...
comparison_data = []
if len(ap_features) > 0:
    for i in range(len(ap_features)):
...
# Get valid results
valid_results = results_df[~results_df['R2_mean'].isna()]

if len(valid_results) > 0:
    best_model = valid_results.loc[valid_results['R2_mean'].idxmax(), 'Model']
    best_r2 = valid_results['R2_mean'].max()
    best_r2_std = valid_results.loc[valid_results['R2_mean'].idxmax(), 'R2_std']
else:
    best_model = "N/A"
    best_r2 = np.nan
    best_r2_std = np.nan
...
Molecules with <4 atoms (TT=0): {np.sum(tt_fps.sum(axis=1) == 0)}
...
if ap_pca is not None:
    report += f"""DIMENSIONALITY REDUCTION
----------------------
...
report += f"""MACHINE LEARNING RESULTS
-----------------------
"""
...
--------------------
Model: {best_model}
R² Score: {best_r2:.4f} ± {best_r2_std:.4f}
"""
...
1. Fingerprint Characteristics:
   - AP captures {np.sum(ap_active)} unique bits ({np.sum(ap_active)/nBits:.1%} of {nBits})
   - TT captures {np.sum(tt_active)} unique bits ({np.sum(tt_active)/nBits:.1%} of {nBits})
   - {len(overlap_bits)} bits are significant for both methods (|r| > {threshold})
   - {np.sum(tt_fps.sum(axis=1) == 0)} molecules have <4 atoms (TT fingerprints are empty)

2. Molecular Encoding:
   - Average AP bits per molecule: {np.mean(np.sum(ap_fps > 0, axis=1)):.1f}
   - Average TT bits per molecule: {np.mean(np.sum(tt_fps > 0, axis=1)):.1f}
   - TT fingerprints require >=4 atoms to generate meaningful patterns

3. Chemical Interpretation:
...
"""
    if len(valid_results) > 1:
        ap_r2 = valid_results[valid_results['Model'].str.contains('AP')]['R2_mean'].values
        tt_r2 = valid_results[valid_results['Model'].str.contains('TT') & ~valid_results['Model'].str.contains('\+')]['R2_mean'].values
        if len(ap_r2) > 0 and len(tt_r2) > 0:
            report += f"""   - {'AP' if np.mean(ap_r2) > np.mean(tt_r2) else 'TT'} fingerprints show stronger correlation with Delta_PCE
"""

report += f"""
RECOMMENDATIONS
--------------
1. For QSPR modeling of Delta_PCE:
"""

if not np.isnan(best_r2):
    report += f"""   - Use {best_model} for best predictive performance
"""
else:
    report += f"""   - Current fingerprint types show limited predictive power
   - Consider combining with other descriptors (physicochemical properties)
"""

report += f"""   - Consider ensemble approaches combining AP and TT fingerprints
   - Filter molecules to those with >=4 atoms for TT fingerprints

2. For molecular interpretation:
   - Use AP fingerprints for identifying key functional groups
   - Use TT fingerprints for understanding conformational effects (when applicable)
   - Focus analysis on top significant bits identified in this study

3. For future studies:
   - Explore larger fingerprint sizes (4096 bits) for more detailed encoding
   - Consider hybrid approaches with Morgan/ECFP fingerprints
   - Investigate the specific chemical meaning of top significant bits
   - Perform substructure analysis to interpret important fingerprint bits
...
print(report)
print("\nAll analysis complete! Results saved to:")
print("  - /share/yhm/test/AutoML_EDA/fingerprints/")
print("  - /share/yhm/test/AutoML_EDA/figures/")
